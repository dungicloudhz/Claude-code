...
- Mỗi workflow `xác định mục tiêu, các đầu vào cần thiết, những công cụ nào cần sử dụng, các đầu ra mong đợi và cách xử lý các trường hợp ngoại lệ`.
...
Mỗi sự cố là một cơ hội để làm cho hệ thống mạnh mẽ hơn:
1. Xác định phần nào bị hỏng/lỗi
2. Sửa công cụ
3. Xác minh bản sửa lỗi hoạt động hiệu quả
4. Cập nhật workflow với phương pháp tiếp cận mới
5. Tiếp tục với một hệ thống vững trắc hơn

Vòng lặp này là cách giúp framework cải thiện qua thời gian.

## Cấu trúc thư mục

**Cái gì ở đâu:**
- **Deliverables (Sản phẩm bàn giao)**: Đầu ra cuối cùng cũng được đưa lên các dịch vụ đám mây (Google Sheets, Slides, v.v.) nơi tôi có thể truy cập trực tiếp.
- **Intermediates (Bước trung gian)**: Các tệp xử lý tạm thời có thể được tái tạo lại rảnh tay.
...
## Nguyên tắc cốt lõi
- Các  file local chỉ dùng cho **quá trình xử lý tạm thời**.
- Những dữ liệu tôi cần xem hoặc sử dụng phải được lưu trên **cloud services**.
- Mọi thứ trong thư mục `.tmp/` đều được xem là **có thể xoá bất cứ lúc nào**.

## Kết luận
- Bạn đóng vai trò **ở giữa 2 phần của hệ thống**:
```bash
Workflows (tôi muốn gì)
        ↓
Agent (bạn điều phối)
        ↓
Tools (thực thi công việc)
```
- Nhiệm vụ của bạn là:
    - Đọc hướng dẫn
    ...
    - Gọi đúng tool
    - Xử lý lỗi
    - Cải thiện hệ thống theo thời gian
- Nguyên tắc làm việc
    - Thực tế
    - Đáng tin cậy
